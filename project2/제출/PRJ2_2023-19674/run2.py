...
def ensure_connection():
    ...
    if conn is None or not conn.is_connected():
            conn = connect(
                host="", #local에서 docker로 mysql 돌림림
                port=3307,
                user='root',
...
def print_DVDs():
    ...
    query = """
        SELECT
            d.d_id, d.d_title, d.d_name, d.age_limit, d.total_borrowed, d.remain_num, d.total_num,
            ROUND(AVG(r.rating), 2) AS avg_rating
        FROM
            DVD d
        LEFT JOIN
            Rating r ON d.d_id = r.d_id
        GROUP BY
            d.d_id
        ORDER BY
            d.d_id;
    ...
    print('--------------------------------------------------------------------------------')
    for row in rows:
        d_id, d_title, d_name, age_limit, total_borrowed, remain_num, total_num, avg_rating = row
        # avg_rating이 None일 경우를 대비하여 처리
        avg_rating_display = avg_rating if avg_rating is not None else 'None'
        print(f"{d_id} {d_title} {d_name} {age_limit} {avg_rating_display} {total_borrowed} {remain_num}")
...
def print_users():
    ...
    query = """
        SELECT
            u.u_id, u.u_name, u.u_age, u.total_borrowed,
            ROUND(AVG(r.rating), 2) AS avg_rating
        FROM
            User u
        LEFT JOIN
            Rating r ON u.u_id = r.u_id
        GROUP BY
            u.u_id
        ORDER BY
            u.u_id;
    ...
    print('--------------------------------------------------------------------------------')
    for row in rows:
        u_id, u_name, u_age, total_borrowed, avg_rating = row
        avg_rating_display = avg_rating if avg_rating is not None else 'None'
        print(f"{u_id} {u_name} {u_age} {avg_rating_display} {total_borrowed}")
...
def print_borrowing_status_for_user():
    ...
    query = """
        SELECT
            d.d_id, d.d_title, d.d_name, d.age_limit,
            ROUND(AVG(r.rating), 2) AS avg_rating
        FROM
            Loan l
        JOIN
            DVD d ON l.d_id = d.d_id
        LEFT JOIN
            Rating r ON d.d_id = r.d_id
        WHERE
            l.u_id = %s
    ...
    print('--------------------------------------------------------------------------------')
    for row in rows:
        d_id, d_title, d_name, age_limit, avg_rating = row
        avg_rating_display = avg_rating if avg_rating is not None else 'None'
        print(f'{d_id} {d_title} {d_name} {age_limit} {avg_rating_display}')
    print('--------------------------------------------------------------------------------') 
